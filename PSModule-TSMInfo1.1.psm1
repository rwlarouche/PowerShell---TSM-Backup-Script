...
function Get-TSMService{
	...
	PROCESS {
		...
		try{
			...
			#Try pinging server
			LogEvent $global:LogFile "Pinging server.."
            if (Test-Connection -ComputerName $Server -Count 2 -Quiet ) {  
                 LogEvent $global:LogFile "Ping successful"
            } 
            else {
    	        ...
    	        CSVEvent $global:CSVFile -ErrorLog $ErrorLog
    	        return
            }
			
			#Check WMI Connection, skip rest of script if unable to WMI
			$WMIConnection = Get-WMIConnection -ErrorLog $ErrorLog
			if ($WMIConnection -eq "false") {
				LogEvent $global:LogFile "ERROR: WMI Connection failed. Skipping machine"
            ...
            $JobIPAddress = Start-Job -Name JobIPAddress -scriptblock {
            	param($ServerName)
            	Get-WmiObject -ComputerName $ServerName Win32_NetworkAdapterConfiguration
            } -args $Server
            
            $jobwait = $JobIPAddress | Wait-Job -Timeout 10
            $WMIService = $JobIPAddress | Receive-Job -ErrorAction "continue"

  	       	if ($JobIPAddress.State -ne "Completed"){ #Add Error to Error table if job fails
			...
			$removejob = $JobIPAddress | Remove-Job -force

            #Get server uptime
            $Server = "WW7WB972NOC007"
            $JobUptime = Start-Job -Name JobUptime -scriptblock {
            	param($ServerName)
            	Get-WmiObject Win32_OperatingSystem -ComputerName $ServerName | foreach {$_.convertToDateTime($_.LastBootUpTime)}
            } -args $Server
  
            $jobwait = $JobUptime | Wait-Job -Timeout 10
            $JobUptimeContents = $JobUptime | Receive-Job -ErrorAction "continue"
			
  	    	if ($JobUptime.State -ne "Completed"){ #Add Error to Error table if job fails
            ...
            Else{
            	#FIX THIS
                $sysUpTime = ((Get-Date) - $JobUptimeContents)
                $uptime = "$($sysuptime.days) Days, $($sysuptime.hours) Hours, $($sysuptime.minutes) Minutes"
                LogEvent $global:LogFile "`tServer Uptime : $uptime"
            }
			$removejob = $JobUptime | Remove-Job -force
	  	    
			#Get local drives on server
			[System.Collections.ArrayList]$DriveNames = @()
            $JobDriveNames = Start-Job -Name JobDriveNames -scriptblock {
            	param($ServerName)
            	Get-WmiObject -ComputerName $ServerName -Class win32_volume
            } -args $Server
  
            $jobwait = $JobDriveNames | Wait-Job -Timeout 10
            $WMIService = $JobDriveNames | Receive-Job -ErrorAction "continue"

  	    	if ($JobDriveNames.State -ne "Completed"){ #Add Error to Error table if job fails
            ...
            Else{
	     	...
	     	$removejob = $JobDriveNames | Remove-Job -force
...
function Get-ServiceInfo{
	...
	PROCESS {
		try{
			...
			$JobGetService = Start-Job -Name JobGetService -scriptblock {
            	param($ServerName, $ServiceName)
            	Get-WmiObject -ComputerName $ServerName -Class Win32_Service -Filter "Name='$ServiceName'"
            } -args ($Server, $ServiceName)
  
            $jobwait = $JobGetService | Wait-Job -Timeout 10
            $WMIService = $JobGetService | Receive-Job -ErrorAction "continue"

	  		if ($JobGetService.State -ne "Completed"){ #Add Error to Error table if job fails
            ...
            Else{
	     	...
	     	$removejob = $JobGetService | Remove-Job -force
			
			#Get Server Parameters from service via Invoke-Command
			$path = "HKLM:\SYSTEM\CurrentControlSet\services\$ServiceName\Parameters"
			$JobParameters = Start-Job -Name JobParameters -scriptblock {
            	param($ServerName, $path)
            	Invoke-Command -ComputerName $ServerName -Command {Get-ItemProperty -Path $args[0]} -ArgumentList $path
            } -args ($Server, $path)
  
            $jobwait = $JobParameters | Wait-Job -Timeout 30
            $WMIService = $JobParameters | Receive-Job -ErrorAction "continue"
			
			if ($JobParameters.State -ne "Completed"){
					LogEvent $global:LogFile "ERROR: Unable to invoke a remote command on machine. Ensure that the Remote Access Service is Running"
					$ErrorLog.Add("ERROR: Unable to invoke a remote command on machine. Ensure that the Remote Access Service is Running") | Out-Null
			...
			$removejob = $JobParameters | Remove-Job -force
			...
			Else{
				## Read TSM Opt File
				$JobReadFile = Start-Job -Name JobReadFile -scriptblock {
          	  		param($ServerName, $FilePath)
            		Invoke-Command -ComputerName $ServerName -Command {Get-Content -Path $args[0]} -ArgumentList $FilePath
            	} -args ($Server, $TSMOptFile)
  
            	$jobwait = $JobReadFile | Wait-Job -Timeout 30
            	$WMIService = $JobParameters | Receive-Job -ErrorAction "continue"

				if ($JobReadFile.State -ne "Completed"){
					LogEvent $global:LogFile "ERROR: Unable to invoke a remote command on machine. Ensure that the Remote Access Service is Running"
					$ErrorLog.Add("ERROR: Unable to invoke a remote command on machine. Ensure that the Remote Access Service is Running") | Out-Null
...
function Get-WMIConnection{
	[System.Collections.ArrayList]$ErrorLog
	
	#Run Get=WmiObject as a job with a timeout to prevent infinite request if unable to reach WMI job
	$WMIJob= Get-WmiObject -ComputerName $Server -Class Win32_Service -Filter "Name='$ServiceName'" -AsJob
	Wait-Job $WMIJob -Timeout 5 | Out-Null	
	$WMIService = Receive-Job -Job $WMIJob
	if ($WMIJob.State -eq "Completed"){ #Add Error to Error table if job fails
	  	return ("true")
	  }
	else{
	  	$ErrorLog.Add("ERROR: WMI Connection failed") | Out-Null
		return ("false")
	}
}
